...
class ModelExtensionPaymentOmise extends Model {
    ...
    public function install() {
        ...
        $this->model_setting_setting->editSetting($this->_group, array(
            'payment_omise_status'        => 0,
            'payment_omise_pkey'          => '',
            'payment_omise_skey'          => '',
            'payment_omise_pkey_test'     => '',
            'payment_omise_skey_test'     => '',
            'payment_omise_test_mode'     => 0,
            'payment_omise_3ds'           => 0,
            'payment_omise_payment_title' => 'Credit Card (Powered by Omise)',
            'payment_omise_auto_capture'  => self::DEFAULT_AUTO_CAPTURE
        ));
    ...
    private function _retrieveOmiseKeys() {
        if ($this->config->get('payment_omise_status') && $this->config->get('payment_omise_test_mode')) {
            $keys = array(
