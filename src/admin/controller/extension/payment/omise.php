...
class ControllerExtensionPaymentOmise extends Controller {
    ...
    private function setBreadcrumb($current = null) {
        ...
        $breadcrumbs[] = array(
            ...
            'href'      => $this->url->link('extension/payment/omise', 'user_token=' . $this->session->data['user_token'], 'SSL'),
            'separator' => ' :: '
        );
    ...
    private function pageDataDashboardTab() {
        ...
        require_once DIR_SYSTEM . 'library/omise.php';
        // $this->load->library('omise');
        ...
        $data['omise_dashboard'] = array(
            'enabled' => $this->config->get('payment_omise_status'),
            'error'   => '',
            'warning' => '',
        );
            ...
            try {
                ...
                if (!OmisePluginHelperCurrency::isSupport($this->config->get('config_currency'))) {
                    $data['omise_dashboard']['warning'] = sprintf(
                        ...
                        $this->url->link('setting/store', 'user_token=' . $this->session->data['user_token'], 'SSL')
                    );
                }

                $data['OmisePluginHelperCurrency'] = new OmisePluginHelperCurrency();
    ...
    private function pageDataSettingTab() {
    	...
    	if (is_null($this->config->get('payment_omise_auto_capture'))) {
    		$omise_auto_capture = $this->model_extension_payment_omise->getDefaultAutoCapture();
    	} else {
    		$omise_auto_capture = $this->config->get('payment_omise_auto_capture');
    	}

        return array(
            'payment_omise_status'        => $this->config->get('payment_omise_status'),
            'payment_omise_test_mode'     => $this->config->get('payment_omise_test_mode'),
            'payment_omise_3ds'           => $this->config->get('payment_omise_3ds'),
            'payment_omise_pkey_test'     => $this->config->get('payment_omise_pkey_test'),
            'payment_omise_skey_test'     => $this->config->get('payment_omise_skey_test'),
            'payment_omise_pkey'          => $this->config->get('payment_omise_pkey'),
            'payment_omise_skey'          => $this->config->get('payment_omise_skey'),
            'payment_omise_payment_title' => $this->config->get('payment_omise_payment_title'),
            'payment_omise_auto_capture'  => $omise_auto_capture
        );
    }
    ...
    private function redirectTo($destination) {
        switch ($destination) {
            case 'omise_dashboard':
                $this->response->redirect($this->url->link('extension/payment/omise', 'user_token=' . $this->session->data['user_token'], 'SSL'));
                break;
    ...
    public function index() {
        ...
        $data = array_merge(
            ...
            array(
                ...
                'action'             => $this->url->link('extension/payment/omise', 'user_token=' . $this->session->data['user_token'], 'SSL'),
                'cancel'             => $this->url->link('extension/payment', 'user_token=' . $this->session->data['user_token'], 'SSL'),
                'transfer_url'       => $this->url->link('extension/payment/omise/submittransfer', 'user_token=' . $this->session->data['user_token'], 'SSL'),
                'versioncheckup_url' => $this->url->link('extension/payment/omise/ajaxversioncheckup', 'user_token=' . $this->session->data['user_token'], 'SSL'),
            )
        );
    ...
    public function updateConfig() {
        ...
        try {
            ...
            if (! empty($update)) {
                $update['payment_omise_3ds'] = isset($update['payment_omise_3ds']) ? $update['payment_omise_3ds'] : 0;

                // Update
                $this->model_setting_setting->editSetting('payment_omise', $update);
