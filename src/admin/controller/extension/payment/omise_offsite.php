...
class ControllerExtensionPaymentOmiseOffsite extends Controller
{
    ...
    private function setBreadcrumb($current = null)
    {
        ...
        $breadcrumbs[] = array(
            ...
            'href'      => $this->url->link('extension/payment/omise_offsite', 'user_token=' . $this->session->data['user_token'], 'SSL'),
            'separator' => ' :: '
        );
    ...
    {
        return array(
            'payment_omise_offsite_status'        => $this->config->get('payment_omise_offsite_status'),
            'payment_omise_offsite_payment_title' => $this->config->get('payment_omise_offsite_payment_title')
        );
    }
    ...
    private function redirectTo($destination)
    {
        switch ($destination) {
            case 'omise_dashboard':
                $this->response->redirect($this->url->link('extension/payment/omise_offsite', 'user_token=' . $this->session->data['user_token'], 'SSL'));
                break;
    ...
    public function action() {
        ...
        return $this->load->view('extension/payment/omise_offsite_action', $data);
    }
    ...
    public function index()
    {
        ...
        $data = array_merge(
            ...
            array(
                ...
                'action'             => $this->url->link('extension/payment/omise_offsite', 'user_token=' . $this->session->data['user_token'], 'SSL'),
        ...
        $this->response->setOutput($this->load->view('extension/payment/omise_offsite', $data));
    }
    ...
    public function updateConfig()
    {
        ...
        try {
            ...
            if (! empty($update)) {
                if ($update['payment_omise_offsite_status'] == 1 && $this->config->get('payment_omise_status') != 1) {
                    throw new Exception($this->language->get('error_need_omise_extension'));
                }
                // Update
                $this->model_setting_setting->editSetting('payment_omise_offsite', $update);
