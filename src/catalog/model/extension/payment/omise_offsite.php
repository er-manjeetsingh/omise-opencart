...
class ModelExtensionPaymentOmiseOffsite extends Model {
    public function getMethod($address, $total) {
        ...
        if ($this->config->get('payment_omise_offsite_payment_title') != "") {
            $payment_title = $this->config->get('payment_omise_offsite_payment_title');
        } else {
        ...
        return array(
            ...
            'title'      => $payment_title,
            'terms'      => '',
            'sort_order' => $this->config->get('payment_omise_offsite_sort_order')
        );
    }
}
