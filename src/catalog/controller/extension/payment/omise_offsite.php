...
class ControllerExtensionPaymentOmiseOffsite extends Controller {
	...
	public function checkout() {
		if (!empty($this->request->post['offsite_provider'])) {
			...
			$omise = array();
			if ($this->config->get('payment_omise_test_mode')) {
				$omise['pkey'] = $this->config->get('payment_omise_pkey_test');
				$omise['skey'] = $this->config->get('payment_omise_skey_test');
			} else {
				$omise['pkey'] = $this->config->get('payment_omise_pkey');
				$omise['skey'] = $this->config->get('payment_omise_skey');
			}
	...
	public function index() {
		...
		if ($order_info) {
			$data = array_merge($data, array(
				...
				'payment_title'    => empty($this->config->get('payment_omise_offsite_payment_title')) ? $this->language->get('text_title') : $this->config->get('payment_omise_offsite_payment_title')
			));
			...
			// }
			return $this->load->view('extension/payment/omise_offsite', $data);
		}
	}
}